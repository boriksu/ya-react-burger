...
export const socketMiddleware = (wsActions: wsActionsTypes): Middleware => {
  ...
  const connect = (url: string, dispatch: AppDispatch, addToken?: boolean) => {
    try {
      let finalUrl = url;
      if (addToken) {
  ...
  const handleMessage = (data: any, dispatch: AppDispatch) => {
    if (!data?.success) {
      ...
      dispatch({ type: wsActions.onError, error: data?.message });
    } else {
      const { success, ...restData } = data;
      dispatch({ type: wsActions.onMessage, message: restData });
    }
  };
  ...
  const setupEventHandlers = (dispatch: AppDispatch) => {
    ...
    state.socket.onmessage = (event) => {
      try {
        ...
        dispatch({ type: wsActions.onError, error: "Invalid JSON format" });
      }
    };
